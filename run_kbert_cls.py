...
class BertClassifier(nn.Module):
    def __init__(self, args, model):
        ...
        self.output_layer_1 = nn.Linear(args.hidden_size + 1, args.hidden_size)
    ...
    def forward(self, src, label, mask, pos=None, vm=None, sentiment_scores=None):
        """
        Args:
        ...
        # Concatenate sentiment scores to the output
        sentiment_scores = sentiment_scores.unsqueeze(1)
        output = torch.cat((output, sentiment_scores), dim=1)
...
def get_sentiment_score(text):
    result = ie(text)
    if result and result[0] and '情感倾向[正向，负向]' in result[0]:
        sentiment = result[0]['情感倾向[正向，负向]'][0]['text']
        score = result[0]['情感倾向[正向，负向]'][0]['probability']
        return score if sentiment == '正向' else -score
    return 0  # Default neutral score if sentiment is not detected
    ...
    for line_id, line in enumerate(sentences):
        ...
        try:
            if len(line) == 2:
                ...
                text_sentiment = get_sentiment_score(line[columns["text_a"]])
                ...
                dataset.append((token_ids, label, mask, pos, vm, text_sentiment))
                ...
                # Get sentiment scores
                text_a_sentiment = get_sentiment_score(line[columns["text_a"]])
                text_b_sentiment = get_sentiment_score(line[columns["text_b"]])
                avg_sentiment = (text_a_sentiment + text_b_sentiment) / 2
                ...
                dataset.append((token_ids, label, mask, pos, vm, avg_sentiment))
                ...
                # Get sentiment scores
                text_a_sentiment = get_sentiment_score(text_a)
                text_b_sentiment = get_sentiment_score(text_b)
                avg_sentiment = (text_a_sentiment + text_b_sentiment) / 2
                ...
                dataset.append((token_ids, label, mask, pos, vm, avg_sentiment, qid))
...
def main():
    ...
    # Datset loader.
    # Modify batch_loader to include sentiment scores
    def batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms, sentiment_scores):
        ...
        for i in range(instances_num // batch_size):
            ...
            sentiment_scores_batch = sentiment_scores[i*batch_size: (i+1)*batch_size]
            yield input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, sentiment_scores_batch
        if instances_num > instances_num // batch_size * batch_size:
            ...
            sentiment_scores_batch = sentiment_scores[instances_num//batch_size*batch_size:]
            yield input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, sentiment_scores_batch
    ...
    def save_logits(epoch, logits, filepath):
        """将模型输出保存到文件"""
        np_logits = logits.detach().cpu().numpy()  # 先调用.detach()然后转移到CPU
        with open(filepath, "ab") as f:
            np.savetxt(f, np_logits)
    ...
    def evaluate(args, is_test, metrics='Acc'):
        ...
        sentiment_scores = torch.FloatTensor([sample[5] for sample in dataset])
        ...
        if not args.mean_reciprocal_rank:
            for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, sentiment_scores_batch) in enumerate(batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms, sentiment_scores)):
                vms_batch = torch.LongTensor(vms_batch)
                sentiment_scores_batch = sentiment_scores_batch.to(device)
                ...
                with torch.no_grad():
                    try:
                        loss, logits = model(input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, sentiment_scores=sentiment_scores_batch)
                    except:
    ...
    sentiment_scores = torch.FloatTensor([example[5] for example in trainset])
    ...
    for epoch in range(1, args.epochs_num+1):
        ...
        for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, sentiment_scores_batch) in enumerate(batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms, sentiment_scores)):
            ...
            loss, _ = model(input_ids_batch, label_ids_batch, mask_ids_batch, pos=pos_ids_batch, vm=vms_batch, sentiment_scores=sentiment_scores_batch)
            if torch.cuda.device_count() > 1:

...
def main():
    ...
    parser.add_argument("--train_path", type=str, required=False, default='./datasets/medical_ner/train.tsv',       # medical_ner  financial_ner
                        help="Path of the trainset.")
    parser.add_argument("--dev_path", type=str, required=False, default='./datasets/medical_ner/dev.tsv',
                        help="Path of the devset.")
    parser.add_argument("--test_path", type=str, required=False, default='./datasets/medical_ner/test.tsv',
    ...
    # kg
    parser.add_argument("--kg_name", required=False, help="KG name or path", default='CnDbpedia')
    ...
    if True:
        args.epochs_num = 5
        args.batch_size = 16
        args.output_model_path = '/home/caoyc/kbert/kbert_medical_ner_CnDbpedia.bin'
        args.Train = False
    ...
    def evaluate(args, is_test):
        ...
        model.eval()
        if not args.Train:
            tf_name = args.output_model_path[:args.output_model_path.rfind('_')]
            tf =  open(tf_name, 'w', encoding='utf-8')
        ...
        for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vm_ids_batch, tag_ids_batch) in enumerate(batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vm_ids, tag_ids)):
            ...
            for j in range(gold.size()[0]):
                if gold[j].item() in begin_ids:
                    ...
                    # gold_entities_pos.append((gold[start].item(), start, end))      # gold[start].item(),
            ...
            for j in range(pred.size()[0]):
                if pred[j].item() in begin_ids and gold[j].item() != labels_map["[PAD]"] and gold[j].item() != labels_map["[ENT]"]:
                    ...
                    # pred_entities_pos.append((pred[start].item(), start, end))      # pred[start].item(),
            ...
            if not args.Train:
                sents = [''.join([vocab.i2w[idx.item()] for idx in sent if idx.item() > 0]) for sent in input_ids_batch[:]]
                adjust_gold_entities_pos = [(a[0] // 256, a[0] % 256, a[1] % 256) for a in gold_entities_pos]
                adjust_pred_entities_pos = [(a[0] // 256, a[0] % 256, a[1] % 256) for a in pred_entities_pos]
                i, j = 0, 0
                for lineidx in range(len(sents)):
                    sents[lineidx] = sents[lineidx].replace('[UNK]', '`')
                    tf.write(sents[lineidx] + '\n')
                    tf.write('gold: ')
                    while i < len(adjust_gold_entities_pos) and adjust_gold_entities_pos[i][0] == lineidx:
                        tf.write(sents[lineidx][adjust_gold_entities_pos[i][1]:adjust_gold_entities_pos[i][2]+1] + '\t')
                        i += 1
                    tf.write('\npred: ')
                    while j < len(adjust_pred_entities_pos) and adjust_pred_entities_pos[j][0] == lineidx:
                        tf.write(sents[lineidx][adjust_pred_entities_pos[j][1]:adjust_pred_entities_pos[j][2]+1] + '\t')
                        j += 1
                    tf.write('\n')
        ...
        f1 = 2*p*r/(p+r)
        print(f'{correct} / {pred_entities_num} = {p}, {correct} / {gold_entities_num} = {r}')
